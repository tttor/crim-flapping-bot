...
int main(void) {
  ...
  uint16_t ch_3_normal_ppm = 1092;// TODO (@tttor) FIX me.
  ...
  while( (rc.read(1) < (ch_1_normal_ppm-RadioControl::kPPMTolerance)) or 
         ...
         (rc.read(2) > (ch_2_normal_ppm+RadioControl::kPPMTolerance)) or
         (rc.read(3) < (ch_2_normal_ppm-RadioControl::kPPMTolerance)) or // ch 3 is quite unique in that its normal position is at the lowest and ..
         (rc.read(3) > (ch_2_normal_ppm+RadioControl::kPPMTolerance)) or // we have to move a bit to get the pin-change interrupt counts
         (rc.read(4) < (ch_2_normal_ppm-RadioControl::kPPMTolerance)) or
         (rc.read(4) > (ch_2_normal_ppm+RadioControl::kPPMTolerance))
       ) {
