...
Pressure::Pressure() {
	for(int i = 0 ; i < 6 ; i++) ac[i] = readP(BMP085_CAL_AC[i]);
	for(int i = 0 ; i < 2 ; i++) b[i] = readP(BMP085_CAL_B[i]);
	mb = readP(BMP085_CAL_MB);
	mc = readP(BMP085_CAL_MC);
	md = readP(BMP085_CAL_MD);
}
...
short Pressure::readP(unsigned char addr) {
	unsigned char buffer[2];
	read(PressureAddress,addr,2,buffer);
...
void Pressure::getRawReading(unsigned int& up,unsigned short& ut) {
	...
	up = 0;
	write(PressureAddress,BMP085_CONTROL,(READ_PRESSURE + (OSS << 6)));
	delay(2 + (3 << OSS));
	read(PressureAddress,BMP085_CONTROL_OUTPUT,3,buffer);
	up = (((unsigned int) buffer[0] << 16) | ((unsigned int) buffer[1] << 8) | (unsigned int) buffer[2]) >> (8-OSS);
	write(PressureAddress,BMP085_CONTROL,READ_TEMPERATURE);
	delay(5);
	read(PressureAddress,BMP085_CONTROL_OUTPUT,2,buffer);
