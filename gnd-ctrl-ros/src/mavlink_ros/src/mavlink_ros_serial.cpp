...
std::string port = "/dev/ttyUSB0";              ///< The serial port name, e.g. /dev/ttyUSB0, /dev/ttyACM0
int baud = 9600;                 ///< The serial baud rate: 9600, 115200
...
void* serial_wait(void* serial_ptr)
{
  ...
  while (1)
  {
    ...
    ROS_DEBUG("Checking for new data on serial port");
    ...
    if (msgReceived)
    {
      ...
      if (verbose || debug)
        ROS_INFO("Received message from serial with ID #%d (sys:%d|comp:%d):\n", message.msgid, message.sysid,
                 message.compid);
      ROS_DEBUG("Received message from serial with ID #%d (sys:%d|comp:%d):\n", message.msgid, message.sysid,
                 message.compid);
      ...
      switch (message.msgid)
      {
        ...
        {
          ROS_DEBUG("MAVLINK_MSG_ID_RC_CHANNELS_RAW");
          if (rc_pub.getNumSubscribers() > 0) 
          {
            mavlink_rc_channels_raw_t rc;
            mavlink_msg_rc_channels_raw_decode(&message, &rc);
            
            mavlink_ros::RadioControlPtr rc_msg(new mavlink_ros::RadioControl);
            
            rc_msg->ch1_raw = rc.chan1_raw; rc_msg->ch2_raw = rc.chan2_raw;
            rc_msg->ch3_raw = rc.chan3_raw; rc_msg->ch4_raw = rc.chan4_raw;
            rc_msg->ch5_raw = rc.chan5_raw; rc_msg->ch6_raw = rc.chan6_raw;
            rc_msg->ch7_raw = rc.chan7_raw; rc_msg->ch8_raw = rc.chan8_raw;
            rc_msg->port = rc.port;
            rc_msg->rssi = rc.rssi;
            
            rc_msg->header.frame_id = frame_id;
            rc_msg->header.seq = rc.time_boot_ms;
        ...
        {
          ROS_DEBUG("MAVLINK_MSG_ID_ATTITUDE");
          ...
          ///*
           ...
           //* TODO @tttor: so far, highres_imu messages can not be decoded correcly
           //*/
        //case MAVLINK_MSG_ID_HIGHRES_IMU:
        //{
          //ROS_DEBUG("MAVLINK_MSG_ID_HIGHRES_IMU:");
