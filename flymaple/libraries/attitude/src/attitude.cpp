...
void Attitude::getZ(Vector<double> & newZ,Vector<double> & deltaTheta) {
  ...
  std::vector<double> retVal = accelerometer.getReading();
	Vector<double> accOfG(3);
  
  accOfG(0) = retVal.at(0);
  accOfG(1) = retVal.at(1);
  accOfG(2) = retVal.at(2);
...
void Attitude::getX(const Vector<double> & newZ,Vector<double> & newX,Vector<double> & deltaTheta) {
	...
	std::vector<double> retVal = compass.getReading();
  Vector<double> northMagneticPole(3);
  
  northMagneticPole(0) = retVal.at(0);
  northMagneticPole(1) = retVal.at(1);
  northMagneticPole(2) = retVal.at(2);
...
void Attitude::getXYZ(Vector<double> & retValX, Vector<double> & retValY, Vector<double> & retValZ) {
  ...
  std::vector<double> retVal = gyroscope.getReading();
  
	Vector<double> angularSpd(3);
  
  angularSpd(0) = retVal.at(0);
  angularSpd(1) = retVal.at(1);
  angularSpd(2) = retVal.at(2);
...
double Attitude::roll(unsigned char type)
{
  if(type == DEG)
    return roll_*180/kPI;
  else
    return roll_;
}

double Attitude::pitch(unsigned char type)
{
  if(type == DEG)
    return pitch_*180/kPI;
  else
    return pitch_;
}

double Attitude::yaw(unsigned char type)
{
  if(type == DEG)
    return yaw_*180/kPI;
  else
